 ...
 * @param transaction A list of TransactionResource objects that represent the details of the transactions that occurred on the specified date. Each TransactionResource contains information such as the amount, currency code, description, owner ID, financial account ID, actor user ID, transaction type, category ID, and the date and time when the transaction was registered. This field is mandatory and must contain at least one TransactionResource for the specified date.
 */
public record TransactionGroupByDateResource(
        LocalDate date,
        List<TransactionResource> transaction
) {
}
